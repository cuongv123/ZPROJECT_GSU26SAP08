...
CLASS lhc_ReportHeader IMPLEMENTATION.
  ...
  METHOD analyzeAndSave.
    ...
    LOOP AT keys INTO DATA(ls_key).
      ...
      DATA(lt_object_map) = lo_engine->zif_recommendation_engine~build_object_mapping(
      ...
      " Đồng bộ chuẩn các trường ls_agg ánh xạ vào EML
      MODIFY ENTITIES OF zi_mig_report_hd IN LOCAL MODE
        ENTITY ReportHeader
          ...
          WITH VALUE #( (
            ...
            AnalyzedBy           = ls_agg-analyzed_by
            AnalyzedAt           = lv_timestamp
          ) )
          ...
          CREATE BY \_ObjectMap
          FIELDS ( SourceObj SourceType TargetObj TargetType RelationType )
          WITH VALUE #( (
            %cid_ref = ls_key-%cid
            %target  = VALUE #( FOR ls_map IN lt_object_map INDEX INTO m (
                         %cid = ls_key-%cid && '_M_' && m
                         SourceObj    = ls_map-source_obj
                         SourceType   = ls_map-source_type
                         TargetObj    = ls_map-target_obj
                         TargetType   = ls_map-target_type
                         RelationType = ls_map-relation_type ) )
          ) )
        MAPPED ls_mapped
        FAILED ls_failed
