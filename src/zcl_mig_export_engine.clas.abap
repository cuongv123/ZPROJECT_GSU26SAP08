...
CLASS zcl_mig_export_engine IMPLEMENTATION.
  ...
  METHOD export_excel.
    ...
    TRY.
        ...
        LOOP AT lt_registry INTO DATA(ls_section).
          ...
          DATA(lv_total_row) = lines( <lt_data> ) + 2.
          ...
          lo_sheet->set_cell(
            ip_column = 2
            ip_row    = lv_total_row
            ip_value  = lines( <lt_data> ) ).

          DATA(lo_bold_style) = lo_excel->add_new_style( ).
          lo_bold_style->font->bold = abap_true.

          lo_sheet->set_cell_style(
            ip_column = 1
            ip_row    = lv_total_row
            ip_style  = lo_bold_style->get_guid( ) ).
          ...
          DO lines( lo_struct->get_components( ) ) TIMES.
            lo_sheet->set_cell_style(
              ip_column = sy-index
              ip_row    = 1
              ip_style  = lo_bold_style->get_guid( ) ).
  ...
  METHOD export_pdf.
    ...
    TRY.
      ...
      CATCH cx_root INTO DATA(lx_block1).
        rs_result-success = abap_false.
        rs_result-message = |BLOCK1_FETCH_ERROR: { lx_block1->get_text( ) }|.
        RETURN.
    ENDTRY.
    ...
    " ===== KHỐI 2: chuẩn bị tham số in + NEW-PAGE PRINT ON =====
    DATA: ls_print_params TYPE pri_params,
    ...
    IF sy-subrc <> 0 OR lv_valid <> 'X'.
      rs_result-success = abap_false.
      rs_result-message = |BLOCK2_PRINTPARAMS_ERROR: subrc={ sy-subrc } valid={ lv_valid }|.
      RETURN.
    ENDIF.
    ...
    TRY.
        NEW-PAGE PRINT ON
          ...
          NO DIALOG.
      CATCH cx_root INTO DATA(lx_block2).
        rs_result-success = abap_false.
        rs_result-message = |BLOCK2_NEWPAGE_ERROR: { lx_block2->get_text( ) }|.
        RETURN.
    ENDTRY.
    ...
    TRY.
      ...
      CATCH cx_root INTO DATA(lx_block3).
        NEW-PAGE PRINT OFF.
        rs_result-success = abap_false.
        rs_result-message = |BLOCK3_WRITE_ERROR: { lx_block3->get_text( ) }|.
        RETURN.
    ENDTRY.
...
BREAK-POINT.
    TRY.
        ...
        IF sy-subrc <> 0.
          rs_result-success = abap_false.
          rs_result-message = |BLOCK4_CONVERT_SUBRC: { sy-subrc }|.
          RETURN.
        ENDIF.

      CATCH cx_root INTO DATA(lx_block4).
        rs_result-success = abap_false.
        rs_result-message = |BLOCK4_CONVERT_ERROR: { lx_block4->get_text( ) }|.
        RETURN.
    ENDTRY.

    IF lv_pdf_bytecount = 0.
      rs_result-success = abap_false.
      rs_result-message = 'PDF conversion returned 0 bytes.'.
      RETURN.
    ENDIF.

    " ===== KHỐI 5: TLINE -> XSTRING =====
    TRY.
        ...
        IF sy-subrc <> 0.
          rs_result-success = abap_false.
          rs_result-message = |BLOCK5_XSTRING_SUBRC: { sy-subrc }|.
          RETURN.
        ENDIF.

      CATCH cx_root INTO DATA(lx_block5).
        rs_result-success = abap_false.
        rs_result-message = |BLOCK5_XSTRING_ERROR: { lx_block5->get_text( ) }|.
        RETURN.
    ENDTRY.
