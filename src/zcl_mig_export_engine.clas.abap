CLASS zcl_mig_export_engine DEFINITION
  ...
  PUBLIC SECTION.
    ...
    TYPES: BEGIN OF ty_pdf_layout,
             ...
             paper_size  TYPE char10,
             orientation TYPE char1,
             font_size   TYPE i,
...
CLASS zcl_mig_export_engine IMPLEMENTATION.
  METHOD fit_to_page_font_size.
    ...
    WHILE lv_candidate >= lc_font_floor.
      ...
      LOOP AT lt_col_weight INTO DATA(lv_wv) FROM 1.
        ...
        DATA(lv_cw) = iv_table_width * lv_wv / lv_total_weight.

        " o du lieu: chu ve bang lv_candidate
        DATA(lv_maxchar_data) = lv_cw * 10 / ( lv_candidate * 7 ) - 1.
        " o tieu de: chu ve lon hon 1pt (dung font_size + 1 nhu trong render_section_pages)
        DATA(lv_maxchar_hdr)  = lv_cw * 10 / ( ( lv_candidate + 1 ) * 7 ) - 1.
  ...
  METHOD resolve_export_filename.
    ...
    " Loại ký tự không hợp lệ trong tên file (khoảng trắng, / \ : * ? " < > |)
    DATA(lo_regex_invalid_char) = cl_abap_regex=>create_pcre( pattern = '[^A-Za-z0-9_\\-]' ).
    REPLACE ALL OCCURRENCES OF REGEX lo_regex_invalid_char IN lv_prefix WITH '_'.
  ...
  METHOD escape_pdf_text.
    ...
    lv_escaped = replace( val = lv_escaped sub = '(' with = '\(' occ = 0 ).
    lv_escaped = replace( val = lv_escaped sub = ')' with = '\)' occ = 0 ).
    DATA(lo_regex_non_printable) = cl_abap_regex=>create_pcre( pattern = '[^ -~]' ).
    REPLACE ALL OCCURRENCES OF REGEX lo_regex_non_printable IN lv_escaped WITH ' '.
    rv_text = lv_escaped.
  ENDMETHOD.
  ...
  METHOD render_section_pages.
    ...
    LOOP AT lt_col_weight INTO DATA(lv_wv2).
      DATA(lv_cw) = lv_table_width * lv_wv2 / lv_total_weight.
      IF lv_cw < 1.
        lv_cw = 1.
      ENDIF.
    ...
    LOOP AT lt_col_widths INTO DATA(lv_cw2).
      DATA(lv_mc) = lv_cw2 * 10 / ( lv_font_size * 7 ) - 1.
      IF lv_mc < 1.
        lv_mc = 1.
      ENDIF.
    ...
    LOOP AT lt_col_widths INTO DATA(lv_cw3).
      DATA(lv_mc_hdr) = lv_cw3 * 10 / ( ( lv_font_size + 1 ) * 7 ) - 1.
      IF lv_mc_hdr < 1.
        lv_mc_hdr = 1.
      ENDIF.
    ...
    DO.
      ...
      DATA(lv_footer_right_width) = strlen( lv_footer_right_text ) * lv_font_size * 7 / 10.
