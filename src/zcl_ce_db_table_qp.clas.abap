...
CLASS zcl_ce_db_table_qp IMPLEMENTATION.
  METHOD if_rap_query_provider~select.
    ...
    "========================================
    DATA:
      lt_final_table TYPE zcl_parser_types=>tt_db_table.
    DATA(lo_engine) =
    ...
    "========================================
    DATA:
  lt_rap_table TYPE TABLE OF zce_db_table.
    LOOP AT lt_final_table INTO DATA(ls_table).
      APPEND VALUE #(
        programname     = ls_table-program_name
        tablename       = ls_table-table_name
        description     = ls_table-table_description
        operations      = ls_table-operations
        recommendation = ls_table-recommendation
        cdscandidate    = ls_table-cds_candidate
        priority        = ls_table-priority
        migrationapproach = ls_table-migration_approach
      ) TO lt_rap_table.
    ENDLOOP.
    ...
    IF lv_top > 0.
      ...
      LOOP AT lt_rap_table INTO DATA(ls_row)
           FROM lv_from
           TO lv_to.
